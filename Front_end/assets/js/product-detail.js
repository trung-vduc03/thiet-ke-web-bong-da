...
function displayProduct() {
    ...
    productRating.textContent =
        `(${currentProduct.rating || 0} đánh giá)`;


    stockStatus.textContent =
        Number(currentProduct.stock || 1) > 0
            ? "Còn hàng"
            : "Hết hàng";
...
function getProductImages() {
    ...
    if (
        ...
        return currentProduct.images.map(
            convertImagePath
        );
...
function renderThumbnails() {
    ...
    currentImages.forEach(
        function (image, index) {
            ...
            img.alt =
                `${currentProduct.name} - ảnh ${index + 1}`;
...
function renderSizeOptions() {
    ...
    sizes.forEach(
        function (size) {
            ...
            sizeOptionsContainer.appendChild(
                button
            );
...
function renderColorOptions() {
    ...
    colors.forEach(
        function (color) {
            ...
            colorOptionsContainer.appendChild(
                button
            );
...
toggleWishlistBtn.addEventListener(
    "click",
    toggleWishlist
);
